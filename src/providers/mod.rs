/*
    Copyright 2025 Konstantin Adamov
    Licenced under MIT Licence (https://opensource.org/licenses/MIT)
*/

pub mod base_converter;
pub mod english_converter;
pub mod french_converter;
pub mod german_converter;
pub mod indonesian_converter;
pub mod italian_converter;
pub mod portuguese_converter;
pub mod russian_converter;
pub mod spanish_converter;
pub mod number_converter;
